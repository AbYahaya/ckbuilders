...
async function main(): Promise<void> {
  ...
  const binaryPath = resolve(config.agreementVersionTypeBinaryPath);
  ...
  console.log(`AGREEMENT_VERSION_TYPE_CODE_HASH=${codeHash}`);
  console.log("AGREEMENT_VERSION_TYPE_HASH_TYPE=data");
  console.log(`AGREEMENT_VERSION_TYPE_TX_HASH=${txHash}`);
  console.log("AGREEMENT_VERSION_TYPE_TX_INDEX=0x0");
}
