...
async function main(): Promise<void> {
  ...
  const binaryPath = resolve(config.agreementAcceptanceTypeBinaryPath);
  ...
  tx.addOutput({ capacity, lock: sender.script }, dataHex);
  ...
  console.log(`AGREEMENT_ACCEPTANCE_TYPE_CODE_HASH=${codeHash}`);
  console.log("AGREEMENT_ACCEPTANCE_TYPE_HASH_TYPE=data");
  console.log(`AGREEMENT_ACCEPTANCE_TYPE_TX_HASH=${txHash}`);
  console.log("AGREEMENT_ACCEPTANCE_TYPE_TX_INDEX=0x0");
}
