...
async function main(): Promise<void> {
  ...
  const binaryPath = resolve(config.amountCountLockBinaryPath);
  ...
  console.log(`AMOUNT_COUNT_LOCK_CODE_HASH=${codeHash}`);
  console.log("AMOUNT_COUNT_LOCK_HASH_TYPE=data");
  console.log(`AMOUNT_COUNT_LOCK_TX_HASH=${txHash}`);
  console.log("AMOUNT_COUNT_LOCK_TX_INDEX=0x0");
}
